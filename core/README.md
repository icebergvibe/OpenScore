...
org.openscore.providers.*    One package per platform — each DTOs + Mapper + Provider:
                             ...
                             football: ligue1, bundesliga, premierleague, efl (Championship, Carabao Cup),
                                       ...
                                       uefa (Champions / Europa / Conference / Nations League)
...
| `shl` | `ShlProvider` (`SportalityProvider`) | live over SSE when the fetcher streams (`LIVE_PUSH`), else polled; the clock is the latest record's time, not a running clock; event/lineup player ids are Statnet ids, roster ids are athlete UUIDs |
| `hockeyallsvenskan` | `HockeyAllsvenskanProvider` | the site has no day route: the whole season is read from the match page and kept as a normalized snapshot (`SeasonScheduleStore`), refreshed after six hours, with due/live/just-finished games re-read one by one; lineups in line/pairing structure from the game page (`LINE_GROUPS`, about two hours before the puck drop); a club's identity and season come out of the snapshot, player profiles are keyed by page slug. The table, squads, the stat leaderboards and the play-by-play answer only a `POST`, so `STANDINGS`, `TEAM_STATS`, `ROSTER` and `LIVE_UPDATES` are claimed only when the fetcher is a `QueryFetcher`; `EVENTS` is claimed either way, because the game page carries the same play-by-play inline. The live flow carries the play-by-play itself, read when the game document moved and otherwise every 30 s, since a penalty does not move the document. No clock anywhere on the site, so no `CLOCK`; push is MQTT with handed-out credentials and stays unwired |
| `chl` | `ChlProvider` | clock is cumulative elapsed play with no play/stop flag, no shots/coordinates, penalty details only as text; a club's fixtures come from its own season file, so CHL games merge into a club page beside its domestic ones |
| `khl` | `KhlProvider` | no clock, period only, and the feed marks an intermission with `period: 10` rather than a period number; no roster/player endpoints, MQTT push not wired |
...
| `bundesliga` | `BundesligaProvider` | Firebase RTDB; whole-minute clock; squads from ESPN through the crosswalk (the DFL's own are key-gated), no player endpoint; live over the match node's SSE stream when the fetcher streams (`LIVE_PUSH`), with the timeline and stats re-read when the streamed score or status gets ahead of them and at most once a minute otherwise (conditional reads, so unchanged is a `304`), else polled |
| `premier-league` | `PremierLeagueProvider` | five small calls per game; own goals credited to the beneficiary from the `events` grouping (the timeline attributes them to the scorer's team); a converted penalty is `PENALTY_SCORED`; `clock` is whole cumulative minutes and steps back from 47 to 45 at the restart |
...
| `allsvenskan`, `superettan`, `svenska-cupen` | `SwedishLeagueProvider` (games, live, events, lineups from `FogisProvider`; standings/team/roster/player from the Sportomedia `AllsvenskanProvider` / `SuperettanProvider`; the cup has no table) | Fogis answers every live tick where Sportomedia rate-limits and freezes its clock, so the games come from Fogis and only the tables from Sportomedia. A linescore's running half comes from the total less the ended halves, since Fogis marks only halves that are over; `SportomediaProvider`, still usable on its own, reads a break from a newest `PERIOD_RESULT` event rather than from `period`, which the feed never clears. Fogis team ids under these league ids (crosswalk namespace `fogis`) |
| `ucl`, `uel`, `uecl`, `unl` | `ChampionsLeagueProvider`, `EuropaLeagueProvider`, `ConferenceLeagueProvider`, `NationsLeagueProvider` (shared `UefaProvider`) | second-precision clock from the timeline's phase markers; squads from ESPN through the crosswalk (UEFA has no squad endpoint); no assists in the MAIN timeline; `live()` polls the `livescore` hash and re-reads the match on change, trusting a new hash only once the match document reports the same phase (the list leads it by 15-20 s); in-play states sampled 2026-09-24 across a whole Nations League matchday, extra time and penalties still unobserved live. The Nations League adds national sides (no `clubId`, no squads), four group tiers and odd-year biennial seasons |
...
`If-None-Match` revalidation, bounded request deadlines, and GET only. Different URLs are
fetched concurrently; overlapping callers of
...
`LivePollBudget` holds the live-loop policy in one place, used by `pollGame()` and by the
providers whose feeds need a loop of their own (MLB's document diffing, UEFA's change hash,
the SHL and Bundesliga streams). It clamps the interval to the 10 s politeness floor, treats a
failed read as no news rather than the end of the game until five come in a row, and stops
after eight hours of polling so a game whose feed never says "final" is not followed for ever.
A stream-backed provider charges its reconnect pauses to the same bound.
