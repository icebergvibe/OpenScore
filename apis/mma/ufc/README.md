...
| GameState | event document | fight `Status`, `FightNightTracking`, event `LiveFightId` | `Final` → FINAL; `Live`/`Over` (or a tracked `round_start`) → LIVE; a tracked walkout or the live pointer → PRE_GAME; else SCHEDULED; event `Canceled` → CANCELLED |
